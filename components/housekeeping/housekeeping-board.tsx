              ...
              <div className="space-y-2 p-2 min-h-[200px]">
                ...
                {list.map((t) => (
                  <Card key={t.id} className="p-3">
                    ...
                    <div className="mt-1 text-xs text-slate-500"><MapPin className="inline h-3 w-3" /> <Link href={`/dashboard/operations/housekeeping`} className="hover:underline">{t.property.name}</Link></div>
                    {t.assignee && <div className="mt-1 text-xs text-slate-500"><User className="inline h-3 w-3" /> {t.assignee.name}</div>}
                    {t.booking && <div className="mt-1 text-xs text-slate-500">→ <Link href={`/dashboard/reservations/bookings/${t.booking.id}`} className="hover:underline">{t.booking.guest.firstName} {t.booking.guest.lastName} ({t.booking.confirmationCode})</Link></div>}

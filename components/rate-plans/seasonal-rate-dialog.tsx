...
export function SeasonalRateDialog({ mode, initial, ratePlans, properties, onClose }: { mode: 'create' | 'edit'; initial?: Initial; ratePlans?: { id: string; name: string }[]; properties?: { id: string; name: string }[]; onClose?: () => void }) {
  ...
  const formInner = (
    <form onSubmit={onSubmit} className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-1.5"><Label htmlFor="startDate">Start date</Label><Input id="startDate" name="startDate" type="date" required defaultValue={initial?.startDate?.slice(0,10)} /></div>
      <div className="space-y-1.5"><Label htmlFor="endDate">End date</Label><Input id="endDate" name="endDate" type="date" required defaultValue={initial?.endDate?.slice(0,10)} /></div>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button size="sm"><Plus className="h-4 w-4" /> Add seasonal rate</Button></DialogTrigger>
      <DialogContent><DialogHeader><DialogTitle>New seasonal rate</DialogTitle></DialogHeader>{formInner}</DialogContent>
    </Dialog>
  );
}

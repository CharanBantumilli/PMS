...
export function OrganizationForm({ initial, usage }: { initial: Initial; usage: Usage }) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body: any = {
      ...
      email: form.get('email'), phone: form.get('phone') || null, website: form.get('website') || null,
      addressLine1: form.get('addressLine1') || null, addressLine2: form.get('addressLine2') || null,
      city: form.get('city') || null, state: form.get('state') || null, postalCode: form.get('postalCode') || null, country: form.get('country') || null,
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5"><Label htmlFor="state">State / Region</Label><Input id="state" name="state" defaultValue={initial.state || ''} /></div>
        <div className="space-y-1.5"><Label htmlFor="postalCode">Postal code</Label><Input id="postalCode" name="postalCode" defaultValue={initial.postalCode || ''} /></div>
        <div className="space-y-1.5"><Label htmlFor="country">Country (ISO)</Label><Input id="country" name="country" defaultValue={initial.country || ''} maxLength={2} placeholder="IN" /></div>

...
import { Plus, BedDouble, Tag } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <Button asChild variant="outline"><Link href="/dashboard/properties/units/types"><Tag className="h-4 w-4" /> Unit types</Link></Button>
          <Button asChild><Link href="/dashboard/properties/units/new"><Plus className="h-4 w-4" /> New unit</Link></Button>
        </div>
      </div>
      ...
      {properties.length === 0 ? (
        ...
        <UnitsBoard
          units={units.map((u) => ({
            ...
            unitType: u.unitType ? { id: u.unitType.id, name: u.unitType.name } : null,
          }))}
          properties={properties.map((p) => ({ id: p.id, name: p.name }))}
          unitTypes={unitTypes.map((t) => ({ id: t.id, name: t.name }))}

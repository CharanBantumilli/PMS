...
export default async function BookingsPage({ searchParams }: { searchParams: { status?: string; q?: string } }) {
  ...
  const bookings = await prisma.booking.findMany({
    where,
    include: { guest: true, unit: true, property: true, ratePlan: true, createdBy: true },
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button asChild><Link href="/dashboard/bookings/new"><Plus className="h-4 w-4" /> New booking</Link></Button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {STATUSES.map((s) => (
          <Link
            key={s || 'all'}
            href={s ? `/dashboard/bookings?status=${s}` : '/dashboard/bookings'}
            className={`rounded-full border px-3 py-1 text-xs font-medium ${(searchParams.status || '') === s ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}
          >
        ...
        <form className="ml-auto">
          <input
            ...
            className="h-9 w-64 rounded-md border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900"
          />
        </form>
      </div>
      ...
      {bookings.length === 0 ? (
        <Card><CardContent className="py-16 text-center">
          ...
          <Button asChild className="mt-4"><Link href="/dashboard/bookings/new">New booking</Link></Button>
        </CardContent></Card>
      ) : (
        <BookingsTable bookings={bookings.map((b) => ({
          ...
          unit: b.unit ? { name: b.unit.name, number: b.unit.number } : null,

...
import { BedDouble, Calendar, Users, DollarSign, TrendingUp, AlertCircle, Sparkles, Wrench } from 'lucide-react';
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Link href="/dashboard/bookings" className="text-sm font-medium text-slate-700 hover:text-slate-900">View all</Link>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Link href="/dashboard/housekeeping" className="flex items-center justify-between rounded-md border p-3 hover:bg-slate-50">
            ...
            <Link href="/dashboard/maintenance" className="flex items-center justify-between rounded-md border p-3 hover:bg-slate-50">
              <div className="flex items-center gap-3">
      ...
      <Card>
        ...
        <CardContent>
          {recentBookings.length === 0 ? (
            <EmptyState label="No bookings yet. Create your first reservation." action={{ label: 'New booking', href: '/dashboard/bookings/new' }} />
          ) : (
                ...
                <tbody className="divide-y">
                  {recentBookings.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-50">
                      <td className="px-2 py-2 font-medium text-slate-900">{b.guest.firstName} {b.guest.lastName}</td>
                      <td className="px-2 py-2 text-slate-600">{b.property.name} {b.unit ? `· ${b.unit.name}` : ''}</td>
                      <td className="px-2 py-2 text-slate-600">{formatDate(b.arrivalDate)} → {formatDate(b.departureDate)} ({b.nights}n)</td>
                      <td className="px-2 py-2 font-medium">{formatCurrency(Number(b.totalAmount), b.currency)}</td>
                      <td className="px-2 py-2"><Badge className={STATUS_COLORS[b.status]}>{statusLabel(b.status)}</Badge></td>
                      <td className="px-2 py-2 text-slate-600">{b.source}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

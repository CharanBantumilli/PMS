  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <a href="/dashboard/notifications" className={`rounded-full border px-3 py-1 text-xs font-medium ${!searchParams.filter ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>All</a>
        <a href="/dashboard/notifications?filter=unread" className={`rounded-full border px-3 py-1 text-xs font-medium ${searchParams.filter === 'unread' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>Unread ({unreadCount})</a>
      </div>

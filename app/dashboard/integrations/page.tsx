...
import { WebhooksList } from '@/components/integrations/webhooks-list';
import { ApiKeysList } from '@/components/integrations/api-keys-list';
import { IntegrationsGrid } from '@/components/integrations/integrations-grid';
import { Plus, Key, Webhook, Plug, BookOpen } from 'lucide-react';
...
export default async function IntegrationsPage() {
  ...
  const [webhooks, apiKeys, integrations, recentDeliveries] = await Promise.all([
    prisma.webhook.findMany({ where: { organizationId: orgId }, orderBy: { createdAt: 'desc' } }),
    ...
    prisma.webhookDelivery.findMany({ where: { organizationId: orgId }, include: { webhook: true }, orderBy: { createdAt: 'desc' }, take: 20 }),
  ]);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-600">Connect external services, manage API keys and configure webhooks.</p>
      </div>
      <Tabs defaultValue="apps">
        ...
        <TabsContent value="webhooks">
          <div className="space-y-3">
            <div className="flex justify-end"><WebhookDialog mode="create" /></div>
            <WebhooksList
              webhooks={webhooks.map((w) => ({ id: w.id, name: w.name, url: w.url, events: w.events, isActive: w.isActive, createdAt: w.createdAt.toISOString() }))}
              deliveries={recentDeliveries.map((d) => ({ id: d.id, webhookId: d.webhookId, webhookName: d.webhook.name, event: d.event, status: d.status, response: d.response, duration: d.duration, createdAt: d.createdAt.toISOString() }))}
            />
          </div>
        </TabsContent>
...
curl -H "Authorization: Bearer pms_live_xxx" \\
  https://app.pms.io/api/v1/bookings

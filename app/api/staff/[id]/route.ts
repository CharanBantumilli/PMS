...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (parsed.data.role === 'OWNER' && ctx.role !== 'OWNER' && !ctx.isSuperAdmin) return jsonError('Only the owner can assign the owner role.', 403);
  if (u.role === 'OWNER' && parsed.data.role !== 'OWNER' && ctx.role !== 'OWNER' && !ctx.isSuperAdmin) return jsonError('Only the owner can change an owner.', 403);

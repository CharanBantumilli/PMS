...
describe('POST /api/organization/plan', () => {
  ...
  it('updates organization limits', async () => {
    vi.mocked(prisma.organization.update).mockResolvedValueOnce({
      id: 'org-1', maxProperties: 10, maxUsers: 25, maxUnits: 1000,
    } as any);
    const req = new NextRequest('http://localhost:3000/api/organization/plan', {
      method: 'POST',
      body: JSON.stringify({ maxProperties: 10, maxUsers: 25, maxUnits: 1000 }),
    });
    const res = await changePlan(req);
    expect(res.status).toBe(200);
  });

  it('rejects invalid input (negative values)', async () => {
    const req = new NextRequest('http://localhost:3000/api/organization/plan', {
      method: 'POST',
      body: JSON.stringify({ maxProperties: -1 }),
    });
    const res = await changePlan(req);
    expect(res.status).toBe(400);
  });
  ...
  it('rejects MANAGER role', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/organization/plan', {
      method: 'POST',
      body: JSON.stringify({ maxProperties: 10 }),
    });
    const res = await changePlan(req);
    expect(res.status).toBe(403);
  });

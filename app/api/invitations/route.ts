...
export async function POST(req: NextRequest) {
  ...
  const invite = await prisma.invitation.create({
    data: { ...parsed.data, email: parsed.data.email.toLowerCase(), organizationId: ctx.organizationId, token, expiresAt, senderId: ctx.userId },
  });
  ...
  try {
    ...
    await notifyByRole({
      ...
      entity: 'Invitation',
      entityId: invite.id,
      actionUrl: `/dashboard/guests-staff`,
    });
  } catch {}

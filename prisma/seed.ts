...
async function main() {
  ...
  const standard = await prisma.unitTypeDefinition.upsert({
    ...
    update: {},
    create: { id: 'seed-standard', organizationId: org.id, name: 'Standard Queen', baseOccupancy: 2, maxOccupancy: 2, bedType: 'QUEEN', bedCount: 1, bathroomCount: 1 },
  });
  const deluxe = await prisma.unitTypeDefinition.upsert({
    where: { id: 'seed-deluxe' },
    update: {},
    create: { id: 'seed-deluxe', organizationId: org.id, name: 'Deluxe King', baseOccupancy: 2, maxOccupancy: 3, bedType: 'KING', bedCount: 1, bathroomCount: 1 },
  });
  const suite = await prisma.unitTypeDefinition.upsert({
    where: { id: 'seed-suite' },
    update: {},
    create: { id: 'seed-suite', organizationId: org.id, name: 'Ocean Suite', baseOccupancy: 2, maxOccupancy: 4, bedType: 'KING', bedCount: 1, bathroomCount: 2 },
  });
  ...
  for (let i = 0; i < 6; i++) {
    ...
    await prisma.booking.upsert({
      ...
      create: {
        ...
        confirmationCode: code, status, source: (['DIRECT','WALK_IN'] as const)[i % 2],
        arrivalDate: arr, departureDate: dep, nights,
        adults: 1 + (i % 3), children: i % 2,
        unitRate: 149, totalAmount: total, taxAmount, discount: 0, paidAmount: status === 'CHECKED_OUT' ? subtotal : 0, balance: status === 'CHECKED_OUT' ? Math.max(0, total - subtotal) : total, currency: 'INR',
      },
    });
  ...
  for (let i = 0; i < hkUnits.length; i++) {
    ...
    const existing = await prisma.housekeepingTask.findFirst({
      where: { organizationId: org.id, unitId: u.id, type: 'FULL_CLEAN' },
    });
    if (!existing) {
  ...
  // Maintenance
  const existingMaint = await prisma.maintenanceTicket.findFirst({
    where: { organizationId: org.id, title: 'Leaking shower head' },
  });
  if (!existingMaint) {
  ...
  const existingExp = await prisma.expense.findFirst({
    where: { organizationId: org.id, description: 'Electricity bill - May' },
  });
  if (!existingExp) {

...
describe('auth integration: apiContext + hasRole', () => {
  ...
  it('hasRole returns true for OWNER on admin checks', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'OWNER', isSuperAdmin: false }, ['OWNER', 'ADMIN'])).toBe(true);
  });

  it('hasRole rejects HOUSEKEEPER on admin checks', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'HOUSEKEEPER', isSuperAdmin: false }, ['OWNER', 'ADMIN'])).toBe(false);
  });

  it('hasRole accepts RECEPTIONIST for booking operations', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'RECEPTIONIST', isSuperAdmin: false }, ['OWNER', 'ADMIN', 'MANAGER', 'RECEPTIONIST'])).toBe(true);
  });

  it('hasRole accepts HOUSEKEEPER for housekeeping operations', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'HOUSEKEEPER', isSuperAdmin: false }, ['OWNER', 'ADMIN', 'MANAGER', 'RECEPTIONIST', 'HOUSEKEEPER'])).toBe(true);
  });

  it('hasRole accepts ACCOUNTANT for finance operations', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'ACCOUNTANT', isSuperAdmin: false }, ['OWNER', 'ADMIN', 'MANAGER', 'ACCOUNTANT'])).toBe(true);
  });
});

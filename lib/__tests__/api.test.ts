...
describe('hasRole', () => {
  it('returns true for super admin regardless of role', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'STAFF', isSuperAdmin: true }, ['OWNER'])).toBe(true);
  });

  it('returns true when role is in the list', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'MANAGER', isSuperAdmin: false }, ['OWNER', 'MANAGER'])).toBe(true);
  });

  it('returns false when role is not in the list', () => {
    expect(hasRole({ organizationId: 'o', userId: 'u', role: 'HOUSEKEEPER', isSuperAdmin: false }, ['OWNER', 'MANAGER'])).toBe(false);
  });
});
